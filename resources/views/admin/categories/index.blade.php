...
@section('content')
    ...
    <div class="bg-white p-6 rounded-2xl shadow border border-gray-200 mb-6 max-w-md">
        <h2 class="text-lg font-semibold text-[#8A5A32] mb-4">Tambah Kategori</h2>

        <form action="{{ route('admin.categories.store') }}" method="POST" class="flex gap-3">
            @csrf
            <input type="text" name="nama" placeholder="Nama Kategori"
                class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-[#8A5A32]" required>

            <button type="submit" class="px-6 py-2 rounded-lg bg-[#8A5A32] text-white font-semibold hover:bg-[#6F4628]">
                Simpan
            </button>
        </form>
    </div>
