    ...
    <main class="flex-1 p-6 md:p-10 transition-all duration-200">
      ...
      <div class="bg-white p-6 rounded-2xl shadow border border-gray-200 mb-6 max-w-md">
          ...
          <form action="{{ route('admin.categories.store') }}" method="POST" class="flex gap-3">
              @csrf
              <input type="text" name="nama" placeholder="Nama Kategori" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-sky-400" required>
          ...
          <table class="w-full text-sm text-gray-700 border-collapse">
              ...
              <tbody>
                  ...
                  <tr class="border-b hover:bg-gray-50">
                      ...
                      <td class="py-3 px-2 font-medium text-gray-800">{{ $kategori->nama }}</td>
                      <td class="py-3 px-2 flex gap-2">
                          ...
                          <div id="modal-edit-{{ $kategori->id }}" class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center">
                              <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                  <button type="button" onclick="document.getElementById('modal-edit-{{ $kategori->id }}').classList.add('hidden')" 
                                  ...
                                  <form action="{{ route('admin.categories.update', $kategori->id) }}" method="POST">
                                      ...
                                      <div>
                                          ...
                                          <input type="text" name="nama" value="{{ $kategori->nama }}" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-yellow-400" required>
                                      </div>
                                      <div class="flex justify-end gap-3 pt-4">
                                          <button type="button" onclick="document.getElementById('modal-edit-{{ $kategori->id }}').classList.add('hidden')" class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100">Batal</button>
                                          <button type="submit" class="px-6 py-2 rounded-lg bg-yellow-500 text-white font-semibold hover:bg-yellow-600">Update</button>
                                      </div>
                                  </form>
                              </div>
                          </div>
                          ...
                          <div id="modal-delete-{{ $kategori->id }}" class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center">
                              <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                  <button type="button" onclick="document.getElementById('modal-delete-{{ $kategori->id }}').classList.add('hidden')" 
                                      class="absolute top-3 right-3 text-gray-500 hover:text-gray-700">
                                      <i class="ri-close-line text-xl"></i>
                                  </button>
                                  <h2 class="text-lg font-semibold text-red-700 mb-4">Konfimasi Hapus</h2>
                                  <p class="mb-6 text-gray-700">Apakah Anda yakin ingin menghapus kategori <strong>{{ $kategori->nama }}</strong>?</p>
                                  <div class="flex justify-end gap-3">
                                      <button type="button" onclick="document.getElementById('modal-delete-{{ $kategori->id }}').classList.add('hidden')" class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100">Batal</button>
                                      <form action="{{ route('admin.categories.destroy', $kategori->id) }}" method="POST">
                                          @csrf
                                          @method('DELETE')
                                          <button type="submit" class="px-6 py-2 rounded-lg bg-red-500 text-white font-semibold hover:bg-red-600">Hapus</button>
                                      </form>
                                  </div>
                              </div>
                          </div>
                  ...
                  @empty
                  <tr>
                      <td colspan="3" class="text-center py-5 text-gray-500">Belum ada kategori.</td>
                  </tr>
                  @endforelse
              </tbody>
          </table>
