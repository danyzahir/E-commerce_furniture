        ...
        <aside class="w-64 bg-gradient-to-b from-sky-700 to-sky-900 text-white hidden md:flex flex-col shadow-lg">
            ...
            <nav class="flex-1 px-4 py-6 space-y-2 text-sm">
              ...
              @if (request()->routeIs('admin.categories.*')) bg-sky-700 @else hover:bg-sky-600 @endif">
                ...
                <a href="{{ route('admin.users.index') }}"
                    class="flex items-center gap-3 px-3 py-2 rounded-lg transition-colors duration-200 hover:bg-sky-600">
            ...
            <div class="px-6 py-4 border-t border-white/10 flex items-center justify-between">
                ...
                <form action="{{ route('logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="flex items-center gap-1 py-2 px-3 bg-sky-800 hover:bg-sky-700 rounded-lg text-sm font-medium">
        ...
        <main class="flex-1 p-6 md:p-10 transition-all duration-200">
            ...
            <div class="bg-white p-6 rounded-2xl shadow border border-gray-200 mb-6 max-w-md">
                ...
                <form action="{{ route('admin.categories.store') }}" method="POST" class="flex gap-3">
                    ...
                    <button type="submit"
                        class="px-6 py-2 rounded-lg bg-sky-700 text-white font-semibold hover:bg-sky-800">Simpan</button>
                </form>
            </div>
                        ...
                        @forelse($categories as $index => $kategori)
                            <tr class="border-b hover:bg-gray-50">
                                ...
                                <td class="py-3 px-2 flex gap-2">
                                    ...
                                    <!-- TOMBOL EDIT -->
                                    <button
                                        onclick="document.getElementById('modal-edit-{{ $kategori->id }}').classList.remove('hidden')"
                                    ...
                                    <div id="modal-edit-{{ $kategori->id }}"
                                        ...
                                        <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                            ...
                                            <form action="{{ route('admin.categories.update', $kategori->id) }}"
                                                method="POST">
                                                @csrf
                                                @method('PUT')
                                                <div>
                                                ...
                                                <div class="flex justify-end gap-3 pt-4">
                                                    <button type="button"
                                                        onclick="document.getElementById('modal-edit-{{ $kategori->id }}').classList.add('hidden')"
                                                        class="px-5 py-2 rounded-lg border border-gray-300 text-gray-600 hover:bg-gray-100">Batal</button>
                                                    <button type="submit"
                                                        class="px-6 py-2 rounded-lg bg-yellow-500 text-white font-semibold hover:bg-yellow-600">Update</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                                    ...
                                    <!-- MODAL DELETE -->
                                    <div id="modal-delete-{{ $kategori->id }}"
                                        class="fixed inset-0 z-50 hidden bg-black/40 flex items-center justify-center">
                                        <div class="bg-white rounded-2xl shadow-lg w-full max-w-md p-6 relative">
                                            ...
                                            <div class="flex items-start gap-3">
                                                ...
                                                <div>
                                                    <h2 class="text-lg font-semibold text-gray-800 mb-1">Konfirmasi
                                                        Hapus</h2>
                                                    <p class="text-sm text-gray-600">
                                                        Apakah Anda yakin ingin menghapus kategori
                                                        <span
                                                            class="font-semibold text-gray-800">"{{ $kategori->nama }}"</span>?
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div class="flex justify-end gap-3 mt-6">
                                                <button type="button"
                                                    onclick="document.getElementById('modal-delete-{{ $kategori->id }}').classList.add('hidden')"
                                                    class="px-5 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-100 font-medium">
                                                    Batal
                                                </button>
