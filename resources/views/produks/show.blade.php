...
@section('content')
    ...
    @if (session('success'))
        ...
        <script>
            setTimeout(() => {
                const alert = document.getElementById('toastSuccess');
                if (alert) {
                    ...
                    @if ($produk->images)
                        @foreach ($produk->images as $image)
                            <img onclick="gantiGambar(this)" 
                                 ...
                                 class="thumb w-24 h-24 object-cover rounded-2xl border-2 
                                        border-gray-300 hover:border-[#B88753] cursor-pointer transition">
                        @endforeach
                    @endif
                </div>
            </div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <input id="qtyInput" type="text" value="1"
                                   class="w-12 h-8 text-center border border-gray-300 rounded-md focus:ring-[#B88753] focus:outline-none">
                        ...
                        <form action="{{ route('cart.add') }}" method="POST">
                            ...
                            <button type="submit"
                                    class="bg-[#8A5A32] hover:bg-[#6F4628] text-white px-8 py-3 rounded-full font-semibold shadow-md transition flex items-center gap-2">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                @foreach ($rekomendasi as $item)
                    <a href="{{ route('produk.show', $item->id) }}"
                        ...
                        @if ($item->diskon)
                            <span
                                class="absolute top-4 left-4 bg-red-600 text-white text-sm font-semibold px-3 py-1 rounded-lg shadow-lg">
                                -{{ $item->diskon }}%
                            </span>
                        @endif
